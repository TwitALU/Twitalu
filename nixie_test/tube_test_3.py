...
with i2c.I2CMaster() as bus:
	...
	time.sleep(1)
	write_register(bus, address_1, command_reg, 0x01)
	time.sleep(1)
	write_register(bus, address_2, command_reg, 0x01)
	time.sleep(1)
	write_register(bus, address_3, command_reg, 0x01)
	time.sleep(1)
	write_register(bus, address_4, command_reg, 0x01)
	time.sleep(1)
	write_register(bus, address_5, command_reg, 0x01)
	print()
	
	for w in range (0, 10):
		print("Cycle: {0}".format(w))
		for q in range (0, 100):
			print("Level: {0}".format(q))
			write_register(bus, address_1, dimmer_reg, q)
			write_register(bus, address_2, dimmer_reg, q)
			write_register(bus, address_3, dimmer_reg, q)
			write_register(bus, address_4, dimmer_reg, q)
			write_register(bus, address_5, dimmer_reg, q)
			time.sleep(0.5)
	...
	# while True:
		...
		# write_register(bus, address_1, gpioa_register, 0b00000000)
		# time.sleep(0.5)
